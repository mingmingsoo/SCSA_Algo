...
# 문제 풀고 나서 기록
    ...
    총 풀이시간 28분
    14~19   : 문제 이해 및 손코딩, 초기 주석(5)
    18~24   : 입력받기(저번 풀이와 달리 2로 패딩 주기)(6)
    24~33   : 로직 설계(7)
                (1) enumerate 안해서 에러나서 디버깅
                (2) horse_grid를 grid라고 써서 에러나서 디버깅
    33~42   : 테케 답이 안나와서 디버깅(9)
                위치,방향 변환해주고 horse_list에 반영해줘야하는데 안해줘서 수정!
  ...
  시간 80 ms
    ...
    회고
    1. 이런 순차(?) 문제에서 1차원 배열로 관리할거면 nr,nc,nd 갱신 확실히 하기!!
        -> 이거 때문에 디버깅 했응게!!
