...
# 문제 풀고 나서 기록
    ...
    문제 종료 16:36
    총 풀이시간 73분
        23~34   : 문제 이해 및 손코딩(11)
        34~37   : 위 기반 주석, 입력받기(3)
        37~51   : 맵 넘버링, 팀 몸통 dict에 담기(bfs) (14)
                    4 없는 팀 넣어보고 확인
                    -> 꼬리가 안담겨서 num_map[nr][nc] = num 를 조건문 안에서 실행되게 변경
        51~03   : 팀 이동 로직 설계(12)
                    여기서 혹시 1-3 인 팀도 있나..? 했지만.. 일단 없다고 가정하고 풀기
        03~22   : 공 굴리는 로직 설계(19)
        22~25   : 턴 수 늘려서 공굴리는 로직 공 위치 잘 되나 확인(3)
        25~36 : 1차 제출 -> 틀렸습니다(11)
                    디버깅..
                    team_num = num_map[mod][i] 라고 되어있어서 수정!!!
    ...
    회고
        1. 1차 틀린이유: 공굴리기에서 for문 변수 위치 실수 으이구 으이구 정신 안차리나
