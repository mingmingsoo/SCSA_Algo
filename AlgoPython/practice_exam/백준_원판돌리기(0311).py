...
# 문제 풀고 나서 기록
    제출 횟수 2회
    문제 시작 16:53
    1차 제출  17:12 [틀린이유: 평균이 // 이였음.. / 인 줄 알고 틀림]
    문제 종료 17:23
    ...
    총 풀이시간 30분
        ...
        01~09   : 인접 찾기(8)
        09~11   : 정규화(2)
        11~12   : 테케 답 안나와서 디버깅(1)
                    d 가 0,1 로 들어와서 1,-1 로 바꿔줌
        12~23   : 틀린 테케로 디버깅(11)
                    1. 인접한 거 찾을 때 0인애들은 제외하게 수정
                    2. 인접한 로직 range 범위 수정 m -> n 수정
                    3. 문제 다시 읽으니 평균이 // 이여서 수정

  메모리 22 MB
  시간 129 ms

    회고
    1. 검증을 안하면 분명히 틀린다. 명심하도록..... 문제좀 여러번 읽어라.;;;
        오히려 1회차 풀이때는 한번에 맞췄는데.... ㅡㅡ 긴장하렴.
