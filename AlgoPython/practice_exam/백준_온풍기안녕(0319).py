...
# 문제 풀고 나서 기록
    ...
    총 풀이시간 68분
        03~11   : 문제 이해 및 손 코딩(8)
        11~53   : 입력받기, 방향별 딕셔너리 만들기(42)
        53~55   : 종료 로직(2)
        55~56   : 가장자리 로직(1)
        56~00   : 냉기 퍼지는 로직(4)
        00~05   : 검증(5)
        05~11 : 틀렸습니다(6)
                테케가 커서 안봤음 -> 테케 만들어서 냉기 퍼지는거 확인

                if not(0 <= nr < n and 0 <= nc < m) or block[r][c][k // 2]:
                    continue

                이렇게 되어있었는데 이러면 안됨!! 바로 밑은 막혀도 왼, 오는 갈 수 있기 때문!! 밑에처럼 수정
  ...
  회고
    1. 처음 풀었을때보다 어렵게 푼듯... 다시 풀어보기 ..
    2. continue 쓸 때 조심해라 아리때도 이랬잖아;;;
역시 어렵군
'''
from collections import deque
